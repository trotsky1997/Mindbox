...
- [ ] 6.1 On first `process` request after enable, run a one-shot probe (spawn `/bin/true` through `build_sandboxed_command`, expect success) and cache the result
- [ ] 6.2 If the probe fails, mark the daemon `process_unsupported` and return `501 {"code":"process_unsupported", ...}` to every subsequent `process` request without spawning anything

## 7. Tests (in `tools-rust/src/main.rs` `#[cfg(test)]`)

- [ ] 7.1 `start` returns a stable `process_id`; subsequent `read`/`wait` route to the same child; non-existent ids return 404
- [ ] 7.2 Per-action validation rejects missing `command` on `start` and missing `process_id` on `write/read/signal/wait/stop`
- [ ] 7.3 `cwd` escaping (absolute, `..`) is rejected with 400 and no child spawned
- [ ] 7.4 Incremental `read`: two reads return two disjoint slices; cursors advance correctly
- [ ] 7.5 Read after exit returns buffered data with `running:false` and `eof_stdout:true`
- [ ] 7.6 Buffer truncation flag: child writes > `TOOLS_PROCESS_BUFFER_BYTES` causes next `read` to set `truncated:true` and return the newest bytes
- [ ] 7.7 `write` with `eof:true` closes stdin; helper binary observes EOF and exits; subsequent `write` is rejected with 400
- [ ] 7.8 `base64` round-trip on `write`/`read` matches helper-binary's view of stdin / our view of stdout
- [ ] 7.9 `signal SIGTERM` against a graceful helper exits with code 0 via `wait`
- [ ] 7.10 `stop` escalates to `SIGKILL` when the helper ignores `SIGTERM`
- [ ] 7.11 `signal` against an already-exited process is a successful no-op response, not 4xx
- [ ] 7.12 `wait` with `timeout_sec` returns `running:true` on timeout, then a follow-up `wait` returns the exit code after the child exits
- [ ] 7.13 `wait` returns `exit_code:42` for a child that exits 42 (non-zero is not an error)
- [ ] 7.14 `list` returns running and exited-but-not-reaped entries for the session, omits other sessions' entries
- [ ] 7.15 `delete_session` with three running children terminates all three and removes their `process_id`s; subsequent action references to those ids return `404`
- [ ] 7.16 Cross-session reuse: a `process_id` from session A passed inside a request to session B returns `404` without touching session A
- [ ] 7.17 Per-session cap enforcement: `(cap+1)`th `start` returns `429` and no child spawned
- [ ] 7.18 Daemon kill switch: with `TOOLS_PROCESS_ENABLED` unset, route returns `404` before any parsing
- [ ] 7.19 `TOOLS_EXPOSE_PROCESS` gate (in `api-rust/src/tools_forward.rs`): forward returns `403` without contacting upstream
- [ ] 7.20 `process_unsupported` probe: forced negative probe via env knob (`TOOLS_PROCESS_FORCE_UNSUPPORTED=1`) causes every action to return `501 process_unsupported`
...
- [ ] 9.1 Once landed and verified, run `/opsx:archive add-process-tool` to move this change to the archived set
